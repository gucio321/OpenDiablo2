...
func (r *RemoteClientConnection) Open(connectionString, saveFilePath string) error {
	...
	r.logger.Info(fmt.Sprintf("Connected to server at %s", r.tcpConnection.RemoteAddr().String()))
	...
	if err != nil {
		r.logger.Error("RemoteClientConnection: error sending PlayerConnectionRequestPacket to server.")
		return err
	}
...
func (r *RemoteClientConnection) serverListener() {
	...
	for {
		...
		if err != nil {
			r.logger.Error(fmt.Sprintf("failed to decode the packet, err: %v", err))
			return
		}

		p, err := r.decodeToPacket(packet.PacketType, string(packet.PacketData))
		if err != nil {
			r.logger.Error(fmt.Sprintln(packet.PacketType, err))
		}
